...
const validar = (cod_cupom, valor, id_estabelecimento, callback) => {
    let ssql = "select c.id_cupom, c.cod_cupom, c.descricao, c.vl_cupom, c.porc_cupom, ";
    ssql += "c.vl_min_pedido, c.dt_validade, c.ind_ativo from tab_cupom c "
    ...
    db.query(ssql, [valor/100, cod_cupom, id_estabelecimento], (err, result) => {
        callback(err, result);
    });
